...
in screennet.main — no GPU, no network, no training loops.
...
def test_get_cache_path_is_deterministic_sha1() -> None:
    filepath = "some/arbitrary/path"
    expected_hash = hashlib.sha1(filepath.encode()).hexdigest()[:10]
    result = m._get_cache_path(filepath)
    assert result.endswith(f"{expected_hash}.pt")
    assert ".torch" in result
    assert "imagefolder" in result
    # expanduser was applied — no leading ~
    assert not result.startswith("~")


def test_get_cache_path_is_stable_across_calls() -> None:
    assert m._get_cache_path("foo") == m._get_cache_path("foo")
...
def test_summary_enum_values() -> None:
    from screennet.main import Summary
...
def test_average_meter_running_average() -> None:
    from screennet.main import AverageMeter
...
def test_average_meter_summary_average() -> None:
    from screennet.main import AverageMeter, Summary
...
def test_progress_meter_batch_fmtstr_pads_correctly() -> None:
    from screennet.main import ProgressMeter
...
def test_progress_meter_display_prints_entries(capsys) -> None:
    from screennet.main import AverageMeter, ProgressMeter
...
def test_progress_meter_display_summary_prints_star(capsys) -> None:
    from screennet.main import AverageMeter, ProgressMeter, Summary
