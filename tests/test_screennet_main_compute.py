"""Tier-2 unit tests for screennet.main: small models + tiny DataLoaders,
...
import screennet.main as m  # noqa: E402
...
# ---------------------------------------------------------------------------
# from_pil_image_to_plt_display  (Agg backend, write to tmp file)
# ---------------------------------------------------------------------------


def test_from_pil_image_to_plt_display_writes_file(tmp_path) -> None:
    img = Image.new("RGB", (16, 12), color=(10, 20, 30))
    pred_dicts = [{"pred_class": "twitter", "pred_prob": 0.88, "time_for_pred": 0.01}]
    out = tmp_path / "plot.png"

    m.from_pil_image_to_plt_display(
        img,
        pred_dicts,
        to_disk=True,
        interactive=False,
...
def test_setup_workspace_noops_when_image_dir_exists(tmp_path, capsys) -> None:
    image_path = tmp_path / "twitter_facebook_tiktok"
    image_path.mkdir()
